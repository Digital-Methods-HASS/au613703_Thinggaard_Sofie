...
qt <- quadrat.test(ppxy, nx = 4, ny = 4) #reduce from 5 to 4, can make more squares e.g 7x7
...
# Run a quadrat test
quadrat.test(p_cluster, alternative = "regular")
```
...
3. p1 is clustered, p2 is clustered #is apparently correct, but i'm not sure I understand why
4. p1 is clustered, p2 is regular

```{r}
#data: Two `ppp` objects
...
#test 4 times
quadrat.test(ppp1, alternative = "clustered") #p-value = 0.02461
quadrat.test(ppp1, alternative = "regular") #p-value = 0.9754

quadrat.test(ppp2, alternative = "clustered") #p-value = 0.0006173
quadrat.test(ppp2, alternative = "regular") #p-value = 0.9994
```
